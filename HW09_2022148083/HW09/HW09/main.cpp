...
int main()
{
    ...
    glm::vec3 lightCenter = glm::vec3(0.0f, 3.0f, 0.0f);

    // circular light source의 반지름
    float lightRadius = 3.0f;

    // circular light source가 향하는 방향 벡터
    glm::vec3 lightDirection = glm::normalize(glm::vec3(-1.0f, -1.0f, -1.0f));

    lightingShader->setVec3("pointLight.center", lightCenter); // circular light source의 중심 좌표
    lightingShader->setFloat("pointLight.radius", lightRadius); // circular light source의 반지름
    lightingShader->setVec3("pointLight.direction", lightDirection); // circular light source가 향하는 방향 벡터


    // create a cubes
    cube = new Cube();
    cylinder = new Cylinder(48);
